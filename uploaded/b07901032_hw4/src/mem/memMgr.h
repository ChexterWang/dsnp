...
//
// TODO: To promote 't' to the nearest multiple of SIZE_T;
// e.g. Let SIZE_T = 8;  toSizeT(7) = 8, toSizeT(12) = 16
#define toSizeT(t)                                 \
({                                                 \
   size_t _t = t;                                  \
   if(_t%SIZE_T) _t = (_t / SIZE_T + 1) * SIZE_T;  \
   _t;                                             \
})

// Define them by SIZE_T and/or SIZE_T_1 MACROs.
//
// TODO: To demote 't' to the nearest multiple of SIZE_T
// e.g. Let SIZE_T = 8;  downtoSizeT(9) = 8, downtoSizeT(100) = 96
#define downtoSizeT(t)                          \
({                                              \
   size_t _t = t;                               \
   if(_t%SIZE_T) _t = (_t / SIZE_T) * SIZE_T;   \
   _t;                                          \
})
...
class MemBlock
{
   ...
   bool getMem(size_t t, T*& ret) {
      // TODO: getMem: get memory from current block, move _ptr for memory.
      t = toSizeT(t);
      ret = (T*)(void*)_ptr;
...
class MemRecycleList
{
   ...
   void reset() {
      // TODO: delete those list blocks that _arrSize%256 = *this->_arrSize
      delete _nextList;
      _nextList = NULL;
      _first = NULL;
   }
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      #endif // MEM_DEBUG
      // TODO: memMgr->reset
      MemBlock<T>* q = _activeBlock->getNextBlock();
      while(q){
         delete _activeBlock;
         _activeBlock = q;
         q = _activeBlock->getNextBlock();
      }
      _activeBlock->reset();
      for(int i = 0; i < R_SIZE; ++i)
         _recycleList[i].reset();
      if(b){
         _activeBlock = new MemBlock<T>(NULL, b);
         _blockSize = b;
      }
   }
   // Called by new
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      // TODO: getMemRecycleList
      if(_recycleList[m].getArrSize() == n) return &_recycleList[m];
      MemRecycleList<T>* now = _recycleList[m].getNextList();
      MemRecycleList<T>* next = _recycleList[m].getNextList();
      while(next){
         if(next->getArrSize() == n) return next;
         now = next;
         next = next->getNextList();
      }
      next = new MemRecycleList<T>(n);
      if(now) now->setNextList(next);
      else _recycleList[m].setNextList(next);
      return next;
   }
   ...
   T* getMem(size_t t) {
      ...
      // TODO: handling the requested memory being too large error
      if(t > _blockSize){
         cerr << "Requested memory (" << t << ") is greater than block size"
         << "(" << _blockSize << "). " << "Exception raised...\n";
         throw(bad_alloc());
      ...
      // TODO: check if there is a good deleted block
      ret = getMemRecycleList(n)->_first;
      if(ret){
         #ifdef MEM_DEBUG
         cout << "Recycled from _recycleList[" << n << "]..." << ret << endl;
         #endif // MEM_DEBUG
         return getMemRecycleList(n)->popFront();
      }
      ...
      //    #endif // MEM_DEBUG
      // TODO: if bigger than remain recycle remain in _activeBlock and open new block
      if(!_activeBlock->getMem(t, ret)){
         if(_activeBlock->getRemainSize() >= S){
            ...
            cout << "Recycling " << ret << " to _recycleList[" << rn << "]\n";
            #endif // MEM_DEBUG
            T* p = (T*)(void*)_activeBlock->_ptr;
            getMemRecycleList(rn)->pushFront(p);
         }
